...
/**
 * 건물 UPSERT: name+address 기준 중복 방지
 */
async function upsertBuilding(client, data) {
...
/**
 * 건물별 하위 데이터(층/편의시설/통계/LIVE) 삽입
 * 기존 데이터 삭제 후 재삽입 (idempotent)
 */
async function seedBuildingDetails(client, buildingId, { floors, facilities, stats, liveFeeds }) {
  // 기존 하위 데이터 삭제 (이 건물에 대해서만)
...
async function seed() {
  ...
  try {
    ...
    // buildings 테이블에 name+address unique 제약이 없으면 추가
    await client.query(`
      DO $$ BEGIN
        IF NOT EXISTS (
